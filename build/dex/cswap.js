...
export class CSwap extends BaseDex {
    constructor(kupoApi) {
        super(kupoApi);
        this.identifier = DEX_IDENTIFIERS.CSWAP;
        /**
         * On-Chain constants.
         */
        this.orderAddress = 'addr1wxaptpmxcxawvr3pzlhgnpmzz3ql43n2tc8mn3av5kx0yzs09tqh8';
        this.poolAddress = 'addr1z8ke0c9p89rjfwmuh98jpt8ky74uy5mffjft3zlcld9h7ml3lmln3mwk0y3zsh3gs3dzqlwa9rjzrxawkwm4udw9axhs6fuu6e';
        this.lpTokenPolicyId = '63';
    ...
    async liquidityPoolFromUtxo(utxo, poolId = '') {
        ...
        const relevantAssets = utxo.amount.filter((assetBalance) => {
            ...
            if (assetBalanceId.slice(56) == this.lpTokenPolicyId) {
        ...
        const liquidityPool = new LiquidityPool(this, identifierToAsset(relevantAssets[assetAIndex].unit), identifierToAsset(relevantAssets[assetBIndex].unit), relevantAssets[assetAIndex].unit === LOVELACE
            ? BigInt(relevantAssets[assetAIndex].quantity) - 2000000n
            : BigInt(relevantAssets[assetAIndex].quantity), BigInt(relevantAssets[assetBIndex].quantity), utxo.address, 0.3, poolId);
    ...
    async liquidityPoolFromPoolId(poolId, initial = false) {
        const utxos = await this.allLiquidityPoolUtxos();
    ...
    async liquidityPoolsFromToken(tokenB, tokenA = LOVELACE, tokenBDecimals = 0, tokenADecimals = 6, allLiquidityPools = [], skipRefetch) {
        ...
        return (await Promise.all(pools.map((pool) => retry(() => this.liquidityPoolFromPoolId(pool.poolId, true), 5, 100))))
