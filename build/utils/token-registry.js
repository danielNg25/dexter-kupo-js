...
export const transformTokenRegistryAsset = (tokenRegistryAsset) => {
    const metadata = {
...
export const fetchAssetMetadata = async (asset, tokenRegistryUrl = CONFIG_TOKEN_REGISTRY_URL, timeout = 10_000 // timeout in milliseconds
    ...
    try {
        ...
        if (data?.name !== undefined && data?.description !== undefined) {
            ...
            console.error(`Failed to fetch metadata for asset ${asset} due to invalid format. Response:`, data);
            return null;
        }
    }
    catch (error) {
        if (error instanceof Error) {
            if (error.name === 'AbortError') {
                console.error(`Request for asset ${asset} timed out after ${timeout}ms`);
                return null;
            }
            else {
                console.error(`Error fetching metadata for asset ${asset}:`, error.message);
            }
        }
        else {
            console.error(`Unexpected error fetching metadata for asset ${asset}:`, error);
        }
