...
function writeVyfinanceDataToFile(
...
export class Vyfinance extends BaseDex {
    ...
    async allLiquidityPoolDatas(): Promise<VyfinancePoolData[]> {
        const res = await fetch(
            'https://api.vyfi.io/lp?networkId=1&v2=true',
            {}
        );

        let data: VyfinancePoolData[] = await res.json();
    ...
    async liquidityPoolFromUtxoExtend(
        ...
        liquidityPool.reserveA =
            ...
            typeof parameters.PoolAssetABarFee === 'string'
                ? liquidityPool.reserveA - BigInt(parameters.PoolAssetABarFee)
                : liquidityPool.reserveA;
        liquidityPool.reserveB =
            typeof parameters.PoolAssetBBarFee === 'number' ||
            typeof parameters.PoolAssetBBarFee === 'string'
                ? liquidityPool.reserveB - BigInt(parameters.PoolAssetBBarFee)
                : liquidityPool.reserveB;
    ...
    async liquidityPoolsFromToken(
        ...
        const poolDatas: Array<VyfinancePoolData> = [
            ...(structuredData[tokenB]?.[tokenA] || []),
            ...(structuredData[tokenA]?.[tokenB] || []),
        ];
